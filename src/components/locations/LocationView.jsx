  ...
  return (
    <div className="overflow-y-auto h-full">
      <div className="p-6 max-w-3xl mx-auto">
        ...
        <hr style={{ borderColor: 'rgba(226,201,126,0.2)', marginBottom: 20 }} />
        ...
        {/* НПС */}
        {l.npcs?.length > 0 && (
          <Section title={`НПС (${l.npcs.length})`}>
            <div className="flex flex-col gap-2">
              {l.npcs.map((npc, i) => (
                <div key={i} className="flex gap-2 px-3 py-2 rounded-lg" style={{ background: 'var(--bg-row)', border: '0.5px solid var(--border)' }}>
                  <span className="font-cinzel text-sm font-semibold shrink-0" style={{ color: 'var(--text)' }}>{npc.name}</span>
                  {npc.description && (
        ...
        {/* Квесты */}
        {l.quests?.length > 0 && (
          <Section title={`Квесты (${l.quests.length})`}>
            <div className="flex flex-col gap-1.5">
              {l.quests.map((q, i) => {
                const st = QUEST_STATUS_MAP[q.status] ?? QUEST_STATUS_MAP['inactive']
                return (
                  <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: 'var(--bg-row)', border: '0.5px solid var(--border)' }}>
        ...
        {/* Точки интереса */}
        {l.points?.length > 0 && (
          <Section title={`Точки интереса (${l.points.length})`}>
            <div className="flex flex-col gap-2">
              {l.points.map((p, i) => (
                <div key={i} className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                  <button
                    className="w-full flex items-center gap-2 px-4 py-3 text-left transition-colors"
                    style={{ background: openPOI === i ? 'rgba(226,201,126,0.08)' : 'var(--bg-row)' }}
                    onClick={() => setOpenPOI(openPOI === i ? null : i)}
                  >
                    {openPOI === i ? <IconChevronDown size={14} style={{ color: 'var(--gold)' }} /> : <IconChevronRight size={14} style={{ color: 'var(--text-muted)' }} />}
                    <span className="font-cinzel text-sm font-semibold" style={{ color: openPOI === i ? 'var(--gold)' : 'var(--text)' }}>{p.title}</span>
                  </button>
                  {openPOI === i && (
                    <div className="px-4 py-3 border-t" style={{ borderColor: 'var(--border)', background: 'var(--bg-deep)' }}>
                      ...
                      {p.npcs?.length > 0 && (
                        <div className="mb-3">
                          ...
                          {p.npcs.map((n, j) => (
                            <div key={j} className="text-sm py-1" style={{ color: 'var(--text-dim)', borderBottom: '0.5px solid var(--border)' }}>
                      ...
                      {p.quests?.length > 0 && (
                        <div>
                          ...
                          {p.quests.map((q, j) => {
                            const st = QUEST_STATUS_MAP[q.status] ?? QUEST_STATUS_MAP['inactive']
                            return (
                              <div key={j} className="flex items-center gap-2 text-sm py-1" style={{ borderBottom: '0.5px solid var(--border)' }}>
                                <span>{st.icon}</span>
...
function Section({ title, children }) {
  return (
    <div className="mb-5">
      <div className="font-cinzel text-xs uppercase tracking-widest mb-2 pb-1" style={{ color: 'var(--gold)', borderBottom: '1px solid rgba(226,201,126,0.2)' }}>
        {title}
      </div>
      {children}
    </div>
